...
const _blobUrlByAudio = new WeakMap();
let _crossfadeAnimationFrame = null;
let _crossfadeTimeout = null;
let _pendingAutoAdvance = null;
// #region debug-point A:reporter
const reportTrackSwitchDebug = (_hypothesisId, _msg, _data = {}) => { };
// #endregion
function getAudioDebugSnapshot(audio, label) {
    if (!audio) {
...
function readOptionSetting(key, fallback, allowed) {
    if (typeof window === 'undefined')
        return fallback;
    const raw = window.localStorage.getItem(key);
...
function scheduleCrossfadeStep(callback) {
...
function resetAndStopAudio(audio, state) {
    if (!audio)
        return;
...
function stopInactiveAudio(audio) {
    if (!audio)
        return;
...
function setAudioSource(audio, track) {
...
function persistLastPlaybackSnapshot(state) {
 ...
 */
function calculateEqualPowerGains(progress) {
 ...
 */
function fadeInAudio(audio, targetVolume, durationMs = 280) {
    if (!audio)
        return;
 ...
 */
function performManualCrossfade(activeAudio, inactiveAudio, nextTrack, nextIndex, queue, history, targetVolume, durationMs, nextSlot, setState, onPlayFailure) {
...
function performDirectTrackSwitch(activeAudio, inactiveAudio, nextTrack, nextIndex, queue, history, targetVolume, fadeDurationMs, state, setState) {
...
export const usePlayerStore = create((set, get) => ({
    ...
    importMetadataMode: readOptionSetting(IMPORT_METADATA_MODE_KEY, 'embedded-first', ['embedded-first', 'filename-first', 'filename-only']),
    duplicateImportBehavior: readOptionSetting(DUPLICATE_IMPORT_BEHAVIOR_KEY, 'skip', ['skip', 'allow']),
    attachSidecarFiles: readBooleanSetting(ATTACH_SIDECAR_FILES_KEY, true),
    lyricsImportMode: readOptionSetting(LYRICS_IMPORT_MODE_KEY, 'embedded', ['embedded', 'sidecar']),
    lyricsTransition: readOptionSetting(LYRICS_TRANSITION_KEY, 'smooth', ['smooth', 'fade', 'slide', 'instant']),
    isFullscreen: false,
    ...
    playTrack: (track, context, isAutoAdvance = false) => {
        ...
        const isExistingQueue = context && (context === state.queue ||
        ...
        const shouldManualCrossfade = !isAutoAdvance &&
        ...
        if (shouldManualCrossfade) {
            ...
            performManualCrossfade(activeAudio, inactiveAudio, track, newIndex, newQueue, history, targetVolume, fadeDurationMs, nextSlot, (newState) => set(newState), () => performDirectTrackSwitch(activeAudio, inactiveAudio, track, newIndex, newQueue, history, targetVolume, 0, state, (newState) => set(newState)));
            return;
        }
        performDirectTrackSwitch(activeAudio, inactiveAudio, track, newIndex, newQueue, history, targetVolume, fadeDurationMs, state, (newState) => set(newState));
    },
    handleTrackEnded: () => {
        const state = get();
    ...
    nextTrack: () => {
        ...
        if (manualFadeDuration > 0 && activeAudio && inactiveAudio && state.isPlaying) {
            ...
            performManualCrossfade(activeAudio, inactiveAudio, nextTrack, nextIndex, queue, history, targetVolume, durationMs, nextSlot, (newState) => set(newState), () => performDirectTrackSwitch(activeAudio, inactiveAudio, nextTrack, nextIndex, queue, history, targetVolume, 0, state, (newState) => set(newState)));
        }
        else {
    ...
    prevTrack: () => {
        ...
        if (queueIndex > 0) {
            ...
            if (manualFadeDuration > 0 && activeAudio && inactiveAudio && state.isPlaying) {
                ...
                const targetVolume = getTargetVolume(state);
                const durationMs = manualFadeDuration * 1000;
                performManualCrossfade(activeAudio, inactiveAudio, prevTrack, prevIndex, queue, history, targetVolume, durationMs, nextSlot, (newState) => set(newState), () => performDirectTrackSwitch(activeAudio, inactiveAudio, prevTrack, prevIndex, queue, history, targetVolume, 0, state, (newState) => set(newState)));
            }
            else {
                // No crossfade, use standard playTrack
    ...
    toggleMute: () => {
        ...
        if (state.isMuted) {
            if (audio)
                audio.volume = clampAudioVolume(state.volume / 100);
            set({ isMuted: false });
        }
        else {
            if (audio)
