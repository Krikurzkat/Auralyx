...
function getInteractionState(element) {
...
function isIgnoredElement(element) {
    if (element.closest('[data-gsap-ignore]'))
        return true;
...
function resolveInteractiveElement(target) {
    ...
    if (!(interactiveElement instanceof HTMLElement))
        return null;
    if (isIgnoredElement(interactiveElement))
        return null;
...
function setHovered(element, hovered) {
    if (!element)
        return;
    const state = getInteractionState(element);
...
function setFocused(element, focused) {
    if (!element)
        return;
    const state = getInteractionState(element);
...
export default function GlobalInteractionEffects({ children }) {
    useEffect(() => {
        ...
        const handlePointerOver = (event) => {
            ...
            if (!nextElement || nextElement === previousElement)
                return;
