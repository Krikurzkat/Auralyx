...
import { RiArrowLeftLine, RiShuffleLine, RiSkipBackFill, RiPlayFill, RiPauseFill, RiSkipForwardFill, RiRepeatLine, RiRepeat2Line, RiRepeatOneLine, RiHeartLine, RiHeartFill, RiVolumeUpLine, RiVolumeMuteLine, RiCarLine, } from 'react-icons/ri';
...
function getFullscreenLayoutProfile() {
    ...
    if (isiPhone && width < 768)
        return 'iphone';
    if (isAndroid && width < 768)
        return 'android';
    if (isIPad || (width >= 768 && width < 1280))
        return 'tablet';
    return 'desktop';
}
export default function FullscreenPlayer() {
    const { currentTrack, isPlaying, progress, currentTime, duration, volume, isMuted, shuffle, repeat, isFullscreenOpen, queue, queueIndex, showLyrics, showQueue, togglePlay, nextTrack, prevTrack, seekTo, toggleMute, toggleShuffle, cycleRepeat, toggleLyrics, toggleQueue, playTrack, setFullscreenOpen, } = usePlayerStore();
    ...
    const [layoutProfile, setLayoutProfile] = useState(() => getFullscreenLayoutProfile());
    const shellRef = useRef(null);
    const coverWrapperRef = useRef(null);
    const driveModeButtonRef = useRef(null);
    const driveModeTransitionRef = useRef(null);
    const titleContainerRef = useRef(null);
    const titleTextRef = useRef(null);
    const previousOpenRef = useRef(isFullscreenOpen);
    const activeTransitionRef = useRef(null);
    const closeCanvasTimeoutRef = useRef(null);
    const previousTrackIdRef = useRef(null);
    const autoTransitionFrameRef = useRef(null);
    const queueAnimationFrameRef = useRef(null);
    // GSAP Tab Transition refs
    const lyricsContainerRef = useRef(null);
    const queueContainerRef = useRef(null);
    const tabPillRef = useRef(null);
    const lyricsTabBtnRef = useRef(null);
    const queueTabBtnRef = useRef(null);
    const tabTransitionTlRef = useRef(null);
    ...
    const { centeredFocusPosition: centeredLyricFocusPosition, activeLyricIndex, } = useFluidLyricMotion(lyrics, currentTime, isPlaying);
    ...
    const fullscreenGlassStyle = useMemo(() => ({
    ...
    const animateModeToggle = (button, hovered, active) => {
        if (!button)
            return;
    ...
    const selectWithinShell = (selector) => {
        if (!shellRef.current)
            return [];
        return Array.from(shellRef.current.querySelectorAll(selector));
    };
    const gsapSetIfPresent = (targets, vars) => {
        if (targets.length > 0) {
    ...
    useLayoutEffect(() => {
        ...
        if (!visualTrack || !isFullscreenOpen || wasOpen)
            return;
        clearCloseCanvasTimeout();
        killActiveTransition();
        setFullscreenInteractable(false);
    ...
    const startCloseTransition = () => {
        if (!visualTrack || !isFullscreenOpen)
            return;
        clearCloseCanvasTimeout();
        killActiveTransition();
        setFullscreenInteractable(false);
    ...
    const runTabTransition = useCallback((toLyrics) => {
        killTabTransition();
        const lyricsEl = lyricsContainerRef.current;
        const queueEl = queueContainerRef.current;
        if (!lyricsEl || !queueEl)
    ...
    const handleTrackClick = (track, clickedIndex) => {
        const coverWrapper = coverWrapperRef.current;
        ...
        const clickedCover = clickedQueueItem.querySelector('img, div');
        if (!clickedCover) {
            playTrack(track, queue);
            return;
        }
        ...
        const coverClone = clickedCover.cloneNode(true);
    ...
    const handleAutoTrackTransition = () => {
        if (!isFullscreenOpen || transitionState !== 'open')
            return;
        animateUpNextQueueItems();
    };
    const toggleDriveMode = () => {
        if (isDriveModeTransitioning)
            return;
        const coverWrapper = coverWrapperRef.current;
        if (!coverWrapper) {
        ...
        if (!isDriveMode) {
            ...
            window.__clearDriveModeTransition = () => {
    ...
    useLayoutEffect(() => {
        if (!isDriveMode && coverWrapperRef.current) {
            const driveModeRect = window.__driveModeBackRect;
            if (driveModeRect) {
                const coverWrapper = coverWrapperRef.current;
                ...
                gsap.to(coverWrapper, {
                    ...
                    onComplete: () => {
                        ...
                        delete window.__driveModeBackRect;
                        setIsDriveModeTransitioning(false);
                    }
                });
    ...
    return (_jsxs("div", { ref: shellRef, className: "fixed inset-0 z-50 overflow-hidden", style: {
            ...
            transform: 'translateZ(0)',
        }, children: [_jsx("div", { className: "absolute inset-0 -z-30", style: fullscreenBackgroundStyle }), _jsx("div", { className: "fullscreen-overlay-bg absolute inset-0 -z-20", style: fullscreenGlassStyle, onClick: (e) => {
                    e.stopPropagation();
                    startCloseTransition();
                }, onMouseDown: (e) => e.stopPropagation(), onTouchStart: (e) => e.stopPropagation() }), _jsx("div", { className: "absolute inset-0 -z-10 pointer-events-none", style: { background: 'linear-gradient(to top, rgba(0,0,0,1) 0%, rgba(0,0,0,0.85) 25%, rgba(0,0,0,0.4) 50%, transparent 75%)' } }), visualTrack ? (_jsxs("div", { className: "absolute inset-0 flex h-full w-full flex-col overflow-hidden p-4 pt-6 md:p-8", style: {
                    ...
                    transform: 'translateZ(0)',
                }, onClick: (e) => e.stopPropagation(), onMouseDown: (e) => e.stopPropagation(), onTouchStart: (e) => e.stopPropagation(), children: [_jsxs("div", { className: "fullscreen-close-btn relative z-20 mb-3 flex w-full items-center justify-between", children: [_jsxs("button", { onClick: startCloseTransition, className: "inline-flex items-center gap-2 rounded-full border border-white/20 bg-white/10 backdrop-blur-xl px-3 py-1.5 text-xs font-semibold text-white shadow-lg transition hover:bg-white/20 hover:border-white/30", "aria-label": "Go back", children: [_jsx(RiArrowLeftLine, { size: 18 }), _jsx("span", { children: "Back" })] }), _jsx("div", { className: "pointer-events-auto flex items-center gap-3", children: _jsxs("button", { ref: driveModeButtonRef, "data-gsap-ignore": true, onMouseEnter: () => animateModeToggle(driveModeButtonRef.current, true, isDriveMode), onMouseLeave: () => animateModeToggle(driveModeButtonRef.current, false, isDriveMode), onPointerDown: () => pressModeToggle(driveModeButtonRef.current), onPointerUp: () => animateModeToggle(driveModeButtonRef.current, true, isDriveMode), onClick: toggleDriveMode, className: `pointer-events-auto inline-flex items-center gap-2 rounded-full border px-3 py-1.5 text-xs font-semibold shadow-lg backdrop-blur-xl transition ${isDriveMode
                                        ? 'border-accent/50 bg-accent/30 text-accent hover:bg-accent/40'
                                        : 'border-white/20 bg-white/10 text-white hover:bg-white/20 hover:border-white/30'}`, "aria-label": "Toggle drive mode", children: [_jsx(RiCarLine, { size: 18 }), _jsx("span", { children: "Drive Mode" })] }) })] }), _jsxs("div", { className: "m-auto h-full w-full max-w-6xl overflow-hidden pt-0 pb-4 flex flex-col landscape:grid landscape:grid-cols-[minmax(360px,448px)_minmax(0,1fr)] landscape:gap-x-12 landscape:items-center landscape:overflow-visible xl:grid xl:grid-cols-[448px_minmax(0,1fr)] xl:gap-x-12 xl:items-center xl:justify-center relative", children: [_jsxs("div", { className: "portrait:contents landscape:flex landscape:flex-col landscape:justify-center landscape:h-full landscape:col-start-1 landscape:row-start-1 landscape:min-w-0 xl:flex xl:flex-col xl:justify-center xl:h-full xl:col-start-1 xl:row-start-1", children: [_jsx("div", { className: "order-1 flex w-full justify-center", children: _jsx("div", { ref: coverWrapperRef, className: "fullscreen-cover-wrapper relative flex h-[44vw] w-[44vw] max-h-[220px] max-w-[220px] flex-shrink-0 items-center justify-center overflow-hidden rounded-[28px] shadow-2xl sm:h-[240px] sm:w-[240px] md:h-[280px] md:w-[280px] xl:h-[360px] xl:w-[360px]", style: {
                                                ...
                                                position: 'relative',
                                                transform: 'translateZ(0)',
                                            }, children: _jsx("div", { className: "absolute inset-0", style: { transform: 'translateZ(0)' }, children: _jsx("div", { className: "fullscreen-cover-2d absolute inset-0", style: { willChange: 'auto' }, children: coverUrl ? (_jsx("img", { src: coverUrl, alt: visualTrack.title, className: "h-full w-full object-cover", style: { transform: 'translateZ(0)' } })) : (_jsx("div", { className: "h-full w-full", style: {
                                                            background: `linear-gradient(135deg, ${visualTrack.coverGradient?.[0] || '#333333'}, ${visualTrack.coverGradient?.[1] || '#222222'})`,
                                                            transform: 'translateZ(0)',
                                                        } })) }) }) }) }), _jsx("div", { className: "order-2 mt-4 flex flex-col items-center landscape:mt-6 xl:mt-8 w-full", children: _jsxs("div", { className: "flex w-full max-w-2xl flex-col items-center text-center", children: [_jsx("div", { ref: titleContainerRef, className: "overflow-hidden w-full px-6 md:px-8 xl:px-0", children: _jsxs("h1", { ref: titleTextRef, className: `fullscreen-track-title text-2xl font-bold leading-tight md:text-3xl ${isTitleOverflowing ? 'animate-marquee inline-block whitespace-nowrap' : 'truncate'}`, children: [_jsx("span", { className: isTitleOverflowing ? 'pr-8' : '', children: visualTrack.title }), isTitleOverflowing && (_jsx("span", { className: "pr-8", "aria-hidden": "true", children: visualTrack.title }))] }) }), _jsx("p", { className: "fullscreen-artist mt-1 text-base text-white/70 md:text-lg truncate w-full", children: visualTrack.artist }), _jsxs("div", { className: "fullscreen-track-badges mt-2 flex flex-wrap items-center justify-center gap-2", children: [_jsxs("span", { className: "rounded-full border border-white/20 bg-white/10 backdrop-blur-xl px-3 py-1 text-[10px] font-semibold uppercase tracking-[0.15em] text-white/80 shadow-md", children: ["Queue ", currentQueuePosition, "/", Math.max(queue.length, 1)] }), _jsx("span", { className: "rounded-full border border-white/20 bg-white/10 backdrop-blur-xl px-3 py-1 text-[10px] font-semibold uppercase tracking-[0.15em] text-white/80 shadow-md", children: visualTrack.isLocal ? 'Local Track' : 'Streaming' }), repeat !== 'off' ? (_jsx("span", { className: "rounded-full border border-accent/40 bg-accent/20 backdrop-blur-xl px-3 py-1 text-[10px] font-semibold uppercase tracking-[0.15em] text-accent shadow-md", children: repeat === 'one' ? 'Repeat One' : 'Repeat All' })) : null] })] }) }), _jsxs("div", { className: "order-4 mt-4 flex w-full flex-col items-center justify-center gap-2 xl:mt-6", children: [_jsxs("div", { className: "fullscreen-progress-bar mt-2 mb-2 shrink-0 self-stretch px-1 md:w-full md:max-w-md md:self-auto md:px-0", children: [_jsx("input", { type: "range", min: 0, max: 100, value: progress, onChange: (event) => seekTo(Number(event.target.value)), className: "accent-track h-1.5 w-full", style: { '--progress': `${progress}%` } }), _jsxs("div", { className: "mt-1 flex justify-between text-xs text-white/50", children: [_jsx("span", { children: formatDuration(Math.floor(currentTime)) }), _jsx("span", { children: formatDuration(Math.floor(duration)) })] })] }), _jsx("div", { className: "fullscreen-controls shrink-0 w-full max-w-md flex justify-center landscape:max-w-full xl:max-w-md", children: _jsxs("div", { className: "flex items-center justify-between sm:justify-center gap-1 sm:gap-4 w-full px-2 xs:px-4 sm:px-0", children: [_jsx("button", { onClick: toggleShuffle, className: `p-1 sm:p-2 transition ${shuffle ? 'text-accent' : 'text-white/50 hover:text-white'}`, children: _jsx(RiShuffleLine, { size: 24 }) }), _jsx("button", { onClick: () => toggleLike(visualTrack.id), className: "p-1 sm:p-2 transition hover:scale-110", children: isLiked ? _jsx(RiHeartFill, { size: 24, className: "text-accent" }) : _jsx(RiHeartLine, { size: 24, className: "text-white/50 hover:text-white" }) }), _jsx("button", { onClick: prevTrack, className: "p-1 sm:p-2 text-white/70 transition hover:text-white", children: _jsx(RiSkipBackFill, { size: 32 }) }), _jsx("button", { onClick: togglePlay, className: "flex h-[60px] w-[60px] sm:h-16 sm:w-16 items-center justify-center rounded-full bg-white text-surface transition hover:scale-105 shrink-0 mx-2 sm:mx-0", children: isPlaying ? _jsx(RiPauseFill, { size: 32 }) : _jsx(RiPlayFill, { size: 32, className: "ml-1" }) }), _jsx("button", { onClick: nextTrack, className: "p-1 sm:p-2 text-white/70 transition hover:text-white", children: _jsx(RiSkipForwardFill, { size: 32 }) }), _jsx("button", { onClick: toggleMute, className: "p-1 sm:p-2 text-white/50 transition hover:text-white", children: isMuted || volume === 0 ? _jsx(RiVolumeMuteLine, { size: 22 }) : _jsx(RiVolumeUpLine, { size: 22 }) }), _jsx("button", { onClick: cycleRepeat, className: `p-1 sm:p-2 transition ${repeat !== 'off' ? 'text-accent' : 'text-white/50 hover:text-white'}`, children: _jsx(RepeatIcon, { size: 24 }) })] }) })] })] }), _jsx("div", { className: "portrait:contents landscape:flex landscape:flex-col landscape:justify-center landscape:h-full landscape:col-start-2 landscape:row-start-1 landscape:min-w-0 landscape:pl-2 xl:flex xl:flex-col xl:justify-center xl:col-start-2 xl:row-start-1 xl:min-w-0 xl:pl-0", children: _jsxs("div", { className: "order-3 mt-3 flex w-full flex-1 flex-col items-center gap-3 min-h-0 md:mt-6 md:gap-6 landscape:mt-0 xl:mt-8", children: [_jsxs("div", { className: "fullscreen-tabs relative z-10 flex w-full max-w-md items-center justify-between gap-3 rounded-2xl border border-white/20 bg-white/10 backdrop-blur-xl p-1.5 shadow-lg", style: {
                                                top: `${fullscreenTabsTopOffset}px`,
                                                transform: 'translateZ(0)',
                                            }, children: [_jsx("div", { ref: tabPillRef, className: "absolute top-1.5 bottom-1.5 left-1.5 rounded-xl bg-white shadow-md z-0 pointer-events-none", style: {
                                                        width: 'calc(50% - 9px)',
                                                        willChange: tabTransitionTlRef.current ? 'transform, width' : 'auto',
                                                        transform: 'translateZ(0)',
                                                    } }), _jsx("button", { ref: lyricsTabBtnRef, onClick: handleShowLyrics, className: "relative z-10 flex flex-1 items-center justify-center rounded-xl px-3 py-2 text-sm font-semibold transition-colors duration-250 select-none outline-none", style: { willChange: 'color' }, children: "Lyrics" }), _jsx("button", { ref: queueTabBtnRef, onClick: handleShowQueue, className: "relative z-10 flex flex-1 items-center justify-center rounded-xl px-3 py-2 text-sm font-semibold transition-colors duration-250 select-none outline-none", style: { willChange: 'color' }, children: "Queue" })] }), _jsxs("div", { className: "fullscreen-lyrics-panel relative flex flex-col flex-1 w-full min-h-0 xl:items-center mt-2 -mb-12", children: [_jsx("div", { ref: lyricsContainerRef, className: "absolute inset-0 flex flex-col items-center w-full h-full", style: {
                                                        willChange: tabTransitionTlRef.current ? 'transform, opacity, filter' : 'auto',
                                                        transform: 'translateZ(0)',
                                                    }, children: _jsx("div", { className: "relative w-full flex-1 max-w-3xl mx-auto", style: {
                                                            ...
                                                            WebkitMaskImage: fullscreenLyricsMask,
                                                                ...
                                                                return (_jsx("div", { className: "absolute w-full max-w-3xl left-1/2 px-8 text-center font-bold", style: {
                                                                        top: '50%',
                                                                        ...
                                                                        pointerEvents: 'none',
                                                                    }, children: line.text || '♪' }, `${line.time}-${actualIndex}`));
                                                            }) })) : (_jsx("div", { className: "absolute inset-0 flex items-center justify-center", children: _jsxs("div", { className: "text-center", children: [_jsx("div", { className: "text-4xl mb-4", children: "\u266A" }), _jsx("p", { className: "text-lg text-white/50", children: "No lyrics available" }), _jsx("p", { className: "text-sm text-white/30 mt-2", children: "Enjoy the music" })] }) })) }) }), _jsxs("div", { ref: queueContainerRef, className: "absolute inset-0 flex flex-col max-w-3xl mx-auto w-full h-full overflow-hidden pb-12", style: {
                                                        willChange: tabTransitionTlRef.current ? 'transform, opacity, filter' : 'auto',
                                                        transform: 'translateZ(0)',
                                                    }, children: [_jsxs("div", { className: "queue-metadata absolute left-0 right-0 z-10 flex items-center justify-between shrink-0 px-2 pt-1", style: { top: `${fullscreenQueueHeaderTopOffset}px` }, children: [_jsx("h2", { className: "text-xs font-bold uppercase tracking-[0.18em] text-white/60", children: "Up Next" }), _jsx("div", { className: "flex items-end gap-1", children: [0, 1, 2, 3].map((bar) => (_jsx("span", { className: `w-1 rounded-full bg-accent/80 ${isPlaying ? 'animate-[bounce_0.9s_ease-in-out_infinite]' : ''}`, style: {
                                                                            height: `${10 + bar * 5}px`,
                                                                            animationDelay: `${bar * 0.12}s`,
                                                                            opacity: isPlaying ? 1 : 0.45,
                                                                        } }, bar))) })] }), _jsx("div", { className: "absolute inset-x-0 bottom-0 overflow-y-auto scrollbar-thin scrollbar-thumb-white/10 [webkit-overflow-scrolling:touch] px-0 pb-12", style: {
                                                                top: `${fullscreenQueueListTopOffset}px`,
                                                                maskImage: fullscreenQueueMask,
                                                                WebkitMaskImage: fullscreenQueueMask,
                                                            }, children: _jsx("div", { className: "space-y-1.2 w-full pt-0.9", children: visibleUpcomingTracks.length > 0 ? (visibleUpcomingTracks.map((track, index) => {
                                                                    const queuedCoverUrl = getTrackCoverUrl(track);
                                                                    return (_jsxs("button", { onClick: () => handleTrackClick(track, index), className: "queue-item flex w-full items-center gap-3 rounded-2xl px-2.5 py-2.5 text-left transition hover:bg-white/10 border border-transparent hover:border-white/5", children: [queuedCoverUrl ? (_jsx("img", { src: queuedCoverUrl, alt: track.title, className: "h-10 w-10 shrink-0 rounded-xl object-cover" })) : (_jsx("div", { className: "h-10 w-10 shrink-0 rounded-xl", style: { background: `linear-gradient(135deg, ${track.coverGradient?.[0] || '#333333'}, ${track.coverGradient?.[1] || '#222222'})` } })), _jsxs("div", { className: "min-w-0 flex-1", children: [_jsx("div", { className: "truncate text-[13px] font-semibold leading-tight text-white", children: track.title }), _jsx("div", { className: "truncate text-[11px] text-white/50", children: track.artist })] }), _jsx("span", { className: "shrink-0 text-[11px] font-semibold text-right text-white/40", children: `#${index + 1}` })] }, `${track.id}-${index}`));
                                                                })) : (_jsx("div", { className: "queue-empty-state mt-8 rounded-2xl bg-white/5 p-6 text-sm text-white/50 text-center max-w-md mx-auto border border-white/10 backdrop-blur-md", children: "No tracks queued after this one." })) }) })] })] })] }) })] })] })) : null, !isDriveMode && (_jsx(UpNextPopup, { track: queue[queueIndex + 1] || (repeat === 'all' ? queue[0] : null), visible: progress >= 75 && progress <= 80 && isFullscreenOpen })), isDriveMode && visualTrack && (_jsx("div", { className: "absolute inset-0 z-40", style: {
                    willChange: isDriveModeTransitioning ? 'opacity, transform' : 'auto',
                    transform: 'translateZ(0)',
                }, children: _jsx(DrivePlayer, { onClose: () => {
                        setIsDriveModeTransitioning(true);
                        setIsDriveMode(false);
                    }, isEmbedded: true }) }))] }));
}
function UpNextPopup({ track, visible, isDriveMode = false }) {
    if (!track)
        return null;
    const coverGradient = track.coverGradient || ['#3b82f6', '#1e3a8a'];
    return (_jsxs("div", { className: `upnext-popup-card ${isDriveMode ? 'drive-mode' : ''} ${visible ? 'visible' : 'hidden'}`, children: [_jsxs("div", { className: "flex items-center justify-between", children: [_jsxs("span", { className: "text-[10px] font-bold uppercase tracking-widest text-accent flex items-center gap-1.5", children: [_jsx("span", { className: "h-1.5 w-1.5 rounded-full bg-accent animate-pulse" }), "Up Next"] }), _jsxs("div", { className: "upnext-soundwave", children: [_jsx("div", { className: "upnext-bar", style: { height: '40%' } }), _jsx("div", { className: "upnext-bar", style: { height: '70%' } }), _jsx("div", { className: "upnext-bar", style: { height: '100%' } }), _jsx("div", { className: "upnext-bar", style: { height: '50%' } })] })] }), _jsxs("div", { className: "flex items-center gap-3", children: [track.coverUrl ? (_jsx("img", { src: track.coverUrl, alt: track.title, className: "h-12 w-12 rounded-xl object-cover shadow-md border border-white/10" })) : (_jsx("div", { className: "h-12 w-12 rounded-xl border border-white/10 shadow-md animate-gradient", style: { background: `linear-gradient(135deg, ${coverGradient[0]}, ${coverGradient[1]})` } })), _jsxs("div", { className: "flex-1 min-w-0", children: [_jsx("div", { className: "text-sm font-bold text-white truncate text-left", children: track.title }), _jsx("div", { className: "text-[11px] text-softText truncate text-left", children: track.artist })] })] })] }));
}
